...
import com.example.owner.fitpro.WorkoutAndExercise.Workout.WorkoutCategory;
...
public abstract class ExpandableRecyclerAdapter<T extends ExpandableRecyclerAdapter.ListItem> extends RecyclerView.Adapter<ExpandableRecyclerAdapter.ViewHolder> {
    ...
    protected static final int TYPE_ROUTINE_HEADER = 1000;
    public static final int TYPE_ROUTINE_EXERCISES = 1001;
    ...
    public boolean toggleExpandedItems(int position, boolean notify) {
        if (isExpanded(position)) {
            ...
            //close the swipe menu if the routine got closed
            ItemTouchHelperExtension i = WorkoutCategory.adapter.getItemTouchHelperExtension();
            i.closeOpened();
            return false;
        } else {
    ...
    public void expandItems(int position, boolean notify) {
        ...
        for (int i=index+1; i<allItems.size() && allItems.get(i).ItemType != TYPE_ROUTINE_HEADER; i++) {
            insert++;
            count++;
    ...
    public void setItems(List<T> items) {
        ...
        for (int i=0; i<items.size(); i++) {
            if (items.get(i).ItemType == TYPE_ROUTINE_HEADER) {
                indexList.add(i);
    ...
    public void collapseAllExcept(int position) {
        for (int i=visibleItems.size()-1; i>=0; i--) {
            if (i != position && getItemViewType(i) == TYPE_ROUTINE_HEADER) {
                if (isExpanded(i)) {
    ...
    public void expandAll() {
        for (int i=visibleItems.size()-1; i>=0; i--) {
            if (getItemViewType(i) == TYPE_ROUTINE_HEADER) {
                if (!isExpanded(i)) {
                    expandItems(i, true);
                }
            }
