package com.example.owner.fitpro.WorkoutAndExercise.Animation;

//Did not import the library in the grandle because i needed to add a method

//Source: https://www.uplabs.com/posts/itemtouchhelper-extension
...
public class ItemTouchHelperExtension extends RecyclerView.ItemDecoration
    ...
    public static final int UP = 1;  //Up direction, used for swipe & drag control.
    public static final int DOWN = 1 << 1; //Down direction, used for swipe & drag control.
    public static final int LEFT = 1 << 2; //Left direction, used for swipe & drag control.
    public static final int RIGHT = 1 << 3; //Right direction, used for swipe & drag control.
    public static final int START = LEFT << 2; //Horizontal start direction. Resolved to LEFT or RIGHT depending on RecyclerView's layout direction. Used for swipe & drag cwontrol.
    public static final int END = RIGHT << 2; //Horizontal end direction. Resolved to LEFT or RIGHT depending on RecyclerView's layout direction. Used for swipe & drag control.
    public static final int ACTION_STATE_IDLE = 0; //ItemTouchHelper is in idle state.
    ...
    public static final int ANIMATION_TYPE_SWIPE_SUCCESS = 1 << 1; //Animation type for views which are swiped successfully.
    public static final int ANIMATION_TYPE_SWIPE_CANCEL = 1 << 2; //Animation type for views which are not completely swiped thus will animate back to their original position.
    public static final int ANIMATION_TYPE_DRAG = 1 << 3; //Animation type for views that were dragged and now will animate to their final position.
    ...
    private static final int PIXELS_PER_SECOND = 1000; //The unit we are using to track velocity
    final List<View> mPendingCleanup = new ArrayList<View>(); //This is necessary after swipe dismissing an item. We wait until animator finishes its job to clean these views.
    private final float[] mTmpPosition = new float[2]; //Re-use array to calculate dx dy for a ViewHolder
    RecyclerView.ViewHolder mSelected = null;  //Currently selected view holder
    RecyclerView.ViewHolder mPreOpened = null;
    float mInitialTouchX; //For drag & drop, this is the time long
    // press is completed vs for swipe, this is the initial touch point
    float mInitialTouchY;
    float mSwipeEscapeVelocity; //Set when ItemTouchHelper is assigned to a RecyclerView.
    float mMaxSwipeVelocity; //Set when ItemTouchHelper is assigned to a RecyclerView.
    float mDx;  //The diff between the last event and initial touch.
    float mDy;
    //The coordinates of the selected view at the time it is selected. We record these values
    //when action starts so that we can consistently position it even if LayoutManager moves the View.
    float mSelectedStartX;
    float mSelectedStartY;
    int mActivePointerId = ACTIVE_POINTER_ID_NONE; //The pointer we are tracking.
    Callback mCallback; //Developer callback which controls the behavior of ItemTouchHelper.
    int mActionState = ACTION_STATE_IDLE; //Current mode.
    int mSelectedFlags; //The direction flags obtained from unmasking
    List<RecoverAnimation> mRecoverAnimations = new ArrayList<RecoverAnimation>();  //When a View is dragged or swiped and needs to go back to where it was, we create a Recover Animation and animate it to its location
    private int mSlop;
    ...
    private VelocityTracker mVelocityTracker; //Used for detecting fling swipe
    private List<ViewHolder> mSwapTargets;  //re-used list for selecting a swap target
    private List<Integer> mDistances; //re used for for sorting swap targets
    private RecyclerView.ChildDrawingOrderCallback mChildDrawingOrderCallback = null; //If drag & drop is supported, we use child drawing order to bring them to front.

    //This keeps a reference to the child dragged by the user. Even after user stops dragging,
    //until view reaches its final position (end of recover animation), we keep a reference so that it can be drawn above other children.
    private View mOverdrawChild = null;
    //We cache the position of the overdraw child to avoid recalculating it each time child
    //position callback is called. This value is invalidated whenever a child is attached or detached.
    private int mOverdrawChildPosition = -1;
    private GestureDetectorCompat mGestureDetector; //Used to detect long press.
    ...
    private final RecyclerView.OnItemTouchListener mOnItemTouchListener
        ...
        public boolean onInterceptTouchEvent(RecyclerView recyclerView, MotionEvent event) {
            ...
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                if (index >= 0) {
                    ...
                    isClosed = checkSelectForSwipe(action, event, index);
                }
            }
        ...
        public void onTouchEvent(RecyclerView recyclerView, MotionEvent event) {
            ...
            if (activePointerIndex >= 0) {
                ...
                isClosed = checkSelectForSwipe(action, event, activePointerIndex);
            }
    ...
    private long mDragScrollStartTimeInMs;  //When user started to drag scroll. Reset when we don't scroll
    ...
    private class RecoverAnimation implements Animator.AnimatorListener {
        ...
        //this important. run twocseperate copies and cpy
        public void cancel() {
    ...
    public void setIsSwipeMenuClosed(boolean isOpen){
        this.isClosed = isOpen;
    }

    public int isSwipeMenuClosed(){
        return mActionState;
    }
}


/* Get back to it when i have time
import android.animation.Animator;
import android.animation.AnimatorListenerAdapter;
import android.animation.ObjectAnimator;
import android.animation.ValueAnimator;
...
public class ItemTouchHelperExtension extends RecyclerView.ItemDecoration
    ...
    //IMPORTANT
    //close the already opened swipe option menu
    private void closeOpenedPreItem() {
        final View view = mCallback.getItemFrontView(mPreOpened);
        if (mPreOpened == null || view == null) return;
        ObjectAnimator objectAnimator = ObjectAnimator.ofFloat(view, "translationX", view.getTranslationX(), 0f);
    ...
    private void select(ViewHolder selected, int actionState) {
        ...
        if (selected != null) {
            mSelectedFlags = (actionStateMask) >> (mActionState * DIRECTION_FLAG_COUNT);
            mSelectedStartX = selected.itemView.getLeft();
            mSelectedStartY = selected.itemView.getTop();
            mSelected = selected;
    ...
    private boolean checkSelectForSwipe(int action, MotionEvent motionEvent, int pointerIndex) {
        ...
        final int swipeFlags = (ACTION_MODE_SWIPE_MASK)
                >> (DIRECTION_FLAG_COUNT * ACTION_STATE_SWIPE);
    ...
    private int checkHorizontalSwipe(int flags) {
        if ((flags & (LEFT | RIGHT)) != 0) {
            final int dirFlag = mDx > 0 ? RIGHT : LEFT;
            final float threshold = getSwipeWidth();
